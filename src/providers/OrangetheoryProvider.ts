 ...
 * Orangetheory is a heart rate-based interval training franchise
 *
 * Note: This implementation is a template based on typical fitness website structures.
 * You'll need to inspect Orangetheory's actual website and adjust selectors.
 */
export class OrangetheoryProvider extends BaseProvider {
  ...
  async scrapeClasses(options: ScrapeOptions): Promise<ScrapeResult> {
    ...
    try {
      ...
      const scheduleUrl = `${this.config.baseUrl}/en-us/studio/${location}`;
      ...
      const classCards = await page.$$('.class-card, .workout-card, .schedule-item, .class-item, .ot-class');
        ...
        try {
          const classInfo = await page.evaluate((card) => {
            ...
            return {
              ...
              instructor: getTextContent('.instructor-name, .coach, .instructor, [data-instructor]'),
              ...
              spotsAvailable: getTextContent('.spots, .availability, .spots-left, .waitlist'),
          ...
          // Orangetheory classes are high-intensity cardio/strength
          const intensity = classInfo.level ? parseIntensity(classInfo.level) : 8;

          // Parse capacity (typical Orangetheory class size)
          const capacity = this.parseCapacity(classInfo.spotsAvailable) || 36;

          // Parse price (Orangetheory typically uses membership model)
          const price = classInfo.price ? this.parsePrice(classInfo.price) : 28;

          // Create fitness class object
          const fitnessClass: FitnessClass = {
            name: sanitizeString(classInfo.name) || 'Orangetheory Workout',
            description: sanitizeString(classInfo.description) || 'Heart rate-based interval training workout',
            datetime,
            location: locationData,
            trainer: sanitizeString(classInfo.instructor) || 'Coach',
            intensity,
            price,
            bookingUrl: this.normalizeUrl(classInfo.bookingLink) || scheduleUrl,
            providerId: `orangetheory-${datetime.getTime()}-${classInfo.name}-${locationName}`,
            providerName: this.name,
            capacity,
            tags: parseTags('hiit cardio strength rowing treadmill interval ' + classInfo.name + ' ' + classInfo.description + ' ' + classInfo.classType)
          };
